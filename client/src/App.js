...
class App extends Component {
  state = { loaded:false, kycAddress: "0x123...", tokenSaleAddress: null, userTokens:0, supply:0 };
  ...
  componentDidMount = async () => {
    try {
      ...
      this.setState({loaded:true, tokenSaleAddress:MyTokenSale.networks[this.networkId].address}, this.updateUserTokens, this.getTotalSupply);
    } catch (error) {
  ...
  getTotalSupply = async () => {
    let supply = await this.tokenInstance.methods.totalSupply().call();
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <p>The Total Token Supply is: {this.state.supply}</p>
        <button type="button" onClick={this.getTotalSupply}>Get Total Supply</button>
      </div>
    );
  }
}
